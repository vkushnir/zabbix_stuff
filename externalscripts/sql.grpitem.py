...
hostid=check_output([SQL, '--login-path=zabbix-login', db_name, '--skip-column-names', '-B', '-s', '-e', "SELECT hostid FROM hosts WHERE host=\"{}\";".format(host)])
if hostid == "":
    print "Error: host \"{}\" not found!!!".format(host)
    sys.exit(1)

str_values=check_output([SQL, '--login-path=zabbix-login', db_name, '--skip-column-names', '-B', '-s', '-e', "SELECT (SELECT value FROM history h WHERE h.itemid=i.itemid ORDER BY clock DESC LIMIT 1) AS value FROM items i WHERE hostid={} AND key_ REGEXP \"{}\";".format(hostid, re_key)]).split()
cnt = len(str_values)
if cnt <= 0:
